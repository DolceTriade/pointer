...
async fn search_symbols(
    ...
    qb.push_bind(request.path.as_deref());
    qb.push(
        ") AS score
         FROM symbols s
    ...
    if include_refs {
        ...
        if !symbol_ids.is_empty() {
            ...
            let ref_rows: Vec<ReferenceRow> = sqlx::query_as(
                "SELECT sr.symbol_id, NULLIF(sn.namespace, '') AS namespace, s.name AS symbol, sr.kind, line_number AS line, column_number AS column
